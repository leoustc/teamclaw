...
type TeamClawSprite = (typeof TEAMCLAW_SPRITES)[number];
...
function spriteSize(sprite: TeamClawSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = TEAMCLAW_SPRITES[(baseCol + baseRow) % TEAMCLAW_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = TEAMCLAW_SPRITES[Math.floor(Math.random() * TEAMCLAW_SPRITES.length)]!;
