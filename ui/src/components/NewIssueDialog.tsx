...
import { TeamClaw } from "./TeamClawIcon";

const DRAFT_KEY = "teamclaw:issue-draft";
...
export function NewIssueDialog() {
  ...
  const defaultManager = useMemo(
    () => (agents ?? []).find((agent) => agent.role === "manager") ?? null,
    [agents],
  );
  const defaultProject = useMemo(
    () => orderedProjects.find((project) => project.name === "default") ?? null,
    [orderedProjects],
  );
  ...
  useEffect(() => {
    if (!newIssueOpen || assigneeId || newIssueDefaults.assigneeAgentId) return;
    if (defaultManager) {
      setAssigneeId(defaultManager.id);
    }
  }, [newIssueOpen, assigneeId, newIssueDefaults.assigneeAgentId, defaultManager]);

  useEffect(() => {
    if (!newIssueOpen || projectId || newIssueDefaults.projectId) return;
    if (defaultProject) {
      setProjectId(defaultProject.id);
    }
  }, [newIssueOpen, projectId, newIssueDefaults.projectId, defaultProject]);
