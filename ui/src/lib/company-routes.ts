import type { Company } from "@teamclawai/shared";
...
export function normalizeCompanyPrefix(prefix: string): string {
  return prefix.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "company";
}

export function companyRouteKey(company: Pick<Company, "name" | "issuePrefix">): string {
  const fromName = normalizeCompanyPrefix(company.name);
  if (fromName) return fromName;
  return normalizeCompanyPrefix(company.issuePrefix);
}
