...
export const authApi = {
  ...
  signInPam: async (input: { username: string; password: string }): Promise<AuthSession | null> => {
    const payload = await authPost("/pam/sign-in", input);
    const direct = toSession(payload);
    if (direct) return direct;
