...
import type { ActivityEvent } from "@teamclawai/shared";
import type { Agent, Issue, IssueAttachment } from "@teamclawai/shared";
import { TeamClaw } from "../components/TeamClawIcon";
...
function formatAction(action: string, details?: Record<string, unknown> | null): string {
  if (action === "issue.monitor.run" && details) {
    const issueRef = typeof details.issueIdentifier === "string" ? details.issueIdentifier : null;
    const proposed = typeof details.proposedStatus === "string" ? details.proposedStatus : null;
    const current = typeof details.currentStatus === "string" ? details.currentStatus : null;
    if (issueRef && proposed) {
      return current
        ? `received handoff about ticket ${issueRef}: current status ${humanizeValue(current)}, proposed status ${humanizeValue(proposed)}`
        : `received handoff about ticket ${issueRef} with proposed status ${humanizeValue(proposed)}`;
    }
  }
  if (action === "issue.monitor.handoff_applied" && details) {
    const issueRef =
      (typeof details.issueIdentifier === "string" && details.issueIdentifier) ||
      (typeof details.receivedIssueRef === "string" && details.receivedIssueRef) ||
      "this issue";
    const previousStatus = typeof details.previousStatus === "string" ? details.previousStatus : null;
    const nextStatus = typeof details.nextStatus === "string" ? details.nextStatus : null;
    const nextAgentName = typeof details.nextAgentName === "string" ? details.nextAgentName : null;
    const nextAgentRole = typeof details.nextAgentRole === "string" ? details.nextAgentRole : null;
    if (nextStatus === "done") {
      return previousStatus
        ? `updated ticket ${issueRef} status from ${humanizeValue(previousStatus)} to done`
        : `updated ticket ${issueRef} status to done`;
    }
    if (nextStatus) {
      return previousStatus
        ? `updated ticket ${issueRef} status from ${humanizeValue(previousStatus)} to ${humanizeValue(nextStatus)} and assigned to ${nextAgentName ?? humanizeValue(nextAgentRole ?? "none")}`
        : `updated ticket ${issueRef} status to ${humanizeValue(nextStatus)} and assigned to ${nextAgentName ?? humanizeValue(nextAgentRole ?? "none")}`;
    }
  }
...
function ActorIdentity({ evt, agentMap }: { evt: ActivityEvent; agentMap: Map<string, Agent> }) {
  ...
  if (evt.actorType === "system") {
    return <Identity name={evt.actorId === "issue-monitor" ? "Monitor" : "System"} size="sm" />;
  }
...
export function IssueDetail() {
  ...
  const lastHasLiveRunsRef = useRef(false);
  ...
  const { data: issue, isLoading, error } = useQuery({
    ...
    refetchInterval: (query) => {
      const currentIssue = query.state.data as Issue | undefined;
      if (!currentIssue) return false;
      return currentIssue.status === "in_progress" || currentIssue.status === "in_review" ? 3000 : false;
    },
  });
  ...
  useEffect(() => {
    const hadLiveRuns = lastHasLiveRunsRef.current;
    lastHasLiveRunsRef.current = hasLiveRuns;
    if (!issueId || !hadLiveRuns || hasLiveRuns) return;
    queryClient.invalidateQueries({ queryKey: queryKeys.issues.detail(issueId) });
    queryClient.invalidateQueries({ queryKey: queryKeys.issues.activity(issueId) });
    queryClient.invalidateQueries({ queryKey: queryKeys.issues.runs(issueId) });
    if (selectedCompanyId) {
  ...
  }, [hasLiveRuns, issueId, queryClient, selectedCompanyId]);
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <TeamClaw className="h-3.5 w-3.5 mr-1.5" />
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        ...
        <TabsContent value="comments">
          <CommentThread
            ...
            agentMap={agentMap}
            draftKey={`teamclaw:issue-comment-draft:${issue.id}`}
            enableReassign
