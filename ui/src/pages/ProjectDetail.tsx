...
import { PROJECT_COLORS, isUuidLike } from "@teamclawai/shared";
...
type ProjectTab = "overview" | "files" | "list" | "configuration";
...
function ProjectFilesExplorer({ projectId, companyId }: { projectId: string; companyId: string }) {
  return (
    <FileExplorer
      scopeKey={`project:${projectId}`}
      title="Files"
      emptyMessage="No files found in the project workspace."
      className="h-[calc(100dvh-17rem)]"
      loadRoot={() => projectsApi.listFiles(projectId, "", companyId)}
      loadChildren={(relativePath) => projectsApi.listFiles(projectId, relativePath, companyId)}
      loadFileContent={(relativePath) => projectsApi.readFile(projectId, relativePath, companyId)}
    />
  );
}
...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey={`teamclaw:project-view:${projectId}`}
...
export function ProjectDetail() {
  ...
  const routeCompanyId = useMemo(() => {
    ...
    return companies.find((company) => companyRouteKey(company) === requestedPrefix)?.id ?? null;
    ...
    <div className="space-y-6">
      ...
      {activeTab === "files" && project?.id && resolvedCompanyId && (
        <div className="max-w-none">
          <ProjectFilesExplorer projectId={project.id} companyId={resolvedCompanyId} />
        </div>
      )}
