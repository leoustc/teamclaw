...
import type { AgentAdapterType, JoinRequest } from "@teamclawai/shared";
...
const ENABLED_INVITE_ADAPTERS = new Set(["claude_local", "codex_local", "opencode_local", "oca_local", "cursor"]);
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">TeamClaw skill bootstrap</p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? "Bootstrap your TeamClaw instance" : "Join this TeamClaw company"}
        </h1>
          ...
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            ...
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>Sign in with PAM</Link>
              </Button>
            </div>
          </div>
        )}
