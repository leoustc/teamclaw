...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@teamclawai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  teamclawUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).teamclawUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    if (!companyId) {
      // Not a live-events websocket path; allow other upgrade handlers to process it.
      return;
    }
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.teamclawUpgradeContext = context;
