...
describe("privateHostnameGuard", () => {
  ...
  it("blocks unknown hostnames with remediation guidance", async () => {
    const app = createApp({ enabled: true, allowedHostnames: ["some-other-host"] });
    const res = await request(app).get("/api/health").set("Host", "dotta-macbook-pro:3100");
    expect(res.status).toBe(403);
    expect(res.body?.error).toContain("Add it to server.allowedHostnames in config.json and restart TeamClaw.");
  });

  it("blocks unknown hostnames on page routes with plain-text remediation guidance", async () => {
    const app = createApp({ enabled: true, allowedHostnames: ["some-other-host"] });
    const res = await request(app).get("/dashboard").set("Host", "dotta-macbook-pro:3100");
    expect(res.status).toBe(403);
    expect(res.text).toContain("Add it to server.allowedHostnames in config.json and restart TeamClaw.");
  });
});
