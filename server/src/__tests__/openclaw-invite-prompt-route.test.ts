...
describe("POST /companies/:companyId/openclaw/invite-prompt", () => {
  ...
  it("rejects non-Manager agent callers", async () => {
    ...
    expect(res.body.error).toContain("Only Manager agents");
  });

  it("allows Manager agent callers and creates an agent-only invite", async () => {
    const db = createDbStub();
    mockAgentService.getById.mockResolvedValue({
      id: "agent-1",
      companyId: "company-1",
