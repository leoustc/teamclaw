...
describe("agent local JWT", () => {
  const secretEnv = "TEAMCLAW_AGENT_JWT_SECRET";
  const ttlEnv = "TEAMCLAW_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "TEAMCLAW_AGENT_JWT_ISSUER";
  const audienceEnv = "TEAMCLAW_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "teamclaw";
    process.env[audienceEnv] = "teamclaw-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
