...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@teamclawai/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@teamclawai/adapter-cursor-local/ui";
import { printCursorStreamEvent } from "@teamclawai/adapter-cursor-local/cli";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$TEAMCLAW_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $TEAMCLAW_API_KEY\"";
