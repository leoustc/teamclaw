...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@teamclawai/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@teamclawai/adapter-codex-local/ui";
import { printCodexStreamEvent } from "@teamclawai/adapter-codex-local/cli";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use teamclaw skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use teamclaw skill**" },
    ]);
  });
