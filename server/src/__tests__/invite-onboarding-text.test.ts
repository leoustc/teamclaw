...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("TeamClaw OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested TeamClaw base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("teamclawApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.teamclawApiUrl");
    expect(text).toContain("~/.openclaw/workspace/teamclaw-claimed-api-key.json");
    expect(text).toContain("TEAMCLAW_API_KEY");
