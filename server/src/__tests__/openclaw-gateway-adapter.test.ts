...
import { execute, testEnvironment } from "@teamclawai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@teamclawai/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@teamclawai/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("teamclaw:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("TEAMCLAW_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("TEAMCLAW_TASK_ID=task-123");
      expect(payload?.teamclaw).toEqual(
