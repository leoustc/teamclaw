...
function blockedHostnameMessage(hostname: string): string {
  return (
    `Hostname '${hostname}' is not allowed for this TeamClaw instance. ` +
    `Add it to server.allowedHostnames in config.json and restart TeamClaw.`
  );
}
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. Add the hostname to server.allowedHostnames in config.json and restart TeamClaw if needed.";
      if (wantsJson) {
