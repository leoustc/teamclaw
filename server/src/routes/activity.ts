...
export function activityRoutes(db: Db) {
  ...
  async function resolveIssueFromRouteId(rawId: string) {
    if (/^[A-Z0-9_]+-\d+$/i.test(rawId)) {
      const byIdentifier = await issueSvc.getByIdentifier(rawId);
  ...
  router.get("/issues/:id/activity", async (req, res) => {
    ...
    const issue = await resolveIssueFromRouteId(rawId);
    if (!issue) {
