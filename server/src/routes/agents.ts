...
import { agents as agentsTable, companies, heartbeatRuns } from "@teamclawai/db";
import { and, count, desc, eq, inArray, ne, not, sql } from "drizzle-orm";
import {
...
import { runClaudeLogin } from "@teamclawai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@teamclawai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@teamclawai/adapter-cursor-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@teamclawai/adapter-opencode-local/server";
import { resolveDefaultAgentWorkspaceDir } from "../home-paths.js";
import { seedRoleTemplateFiles } from "../role-templates.js";
import { ensureInitialAgentSetupIssue } from "../initial-agent-issues.js";
...
export function agentRoutes(db: Db) {
  const DEFAULT_INSTRUCTIONS_PATH_KEYS: Record<string, string> = {
    ...
    oca_local: "instructionsFilePath",
    cline_local: "instructionsFilePath",
    cline_oca_local: "instructionsFilePath",
  ...
  const strictSecretsMode = process.env.TEAMCLAW_SECRETS_STRICT_MODE === "true";
  ...
  async function assertCanCreateAgentsForCompany(
    req: Request,
    companyId: string,
    opts: { requestedRole?: string | null; companyEngineerHeadcount?: number | null } = {},
  ) {
    ...
    const architectCanHireEngineerWithinHeadcount =
      actorAgent.role === "architect" &&
      opts.requestedRole === "engineer" &&
      (await countActiveEngineers(companyId)) < (opts.companyEngineerHeadcount ?? 3);
    if (!allowedByGrant && !canCreateAgents(actorAgent) && !architectCanHireEngineerWithinHeadcount) {
  ...
  async function assertCanUpdateAgent(req: Request, targetAgent: { id: string; companyId: string }) {
    ...
    if (actorAgent.role === "manager") return;
    ...
    throw forbidden("Only Manager or agent creators can modify other agents");
  }
  ...
  async function countActiveEngineers(companyId: string) {
    const row = await db
      .select({ count: count() })
      .from(agentsTable)
      .where(and(eq(agentsTable.companyId, companyId), eq(agentsTable.role, "engineer"), ne(agentsTable.status, "terminated")))
      .then((rows) => rows[0] ?? null);
    return row?.count ?? 0;
  }
  ...
  async function resolveDefaultReportsTo(
    ...
    requestedReportsTo: string | null | undefined,
  ) {
    if (requestedReportsTo) return requestedReportsTo;
    if (role !== "engineer") return requestedReportsTo ?? null;
  ...
  async function ensureAdapterWorkingDirectory(
    adapterType: string | null | undefined,
    adapterConfig: Record<string, unknown>,
    options: {
      companyId?: string;
    ...
    if (!adapterType || !LOCAL_ADAPTER_TYPES.has(adapterType)) return adapterConfig;
    const configuredCwd =
      asNonEmptyString(adapterConfig.cwd) ??
      (options.companyId
        ? resolveDefaultAgentWorkspaceDir(options.companyId, {
            companyName: options.companyName,
            agentRole: options.agentRole,
            agentName: options.agentName,
            agentIdFallback: options.agentIdFallback,
          })
        : null);
    if (!configuredCwd) return adapterConfig;

    const normalizedCwd = normalizeCwdInput(configuredCwd);
    if (!path.isAbsolute(normalizedCwd)) return adapterConfig;

    try {
      await fs.mkdir(normalizedCwd, { recursive: true });
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      throw unprocessable(`Could not create adapterConfig.cwd directory: ${reason}`);
    }

    if (normalizedCwd === configuredCwd && configuredCwd === adapterConfig.cwd) return adapterConfig;
    return { ...adapterConfig, cwd: normalizedCwd };
  }

  async function initializeAgentRoleWorkspace(
    adapterType: string | null | undefined,
    role: string | null | undefined,
    adapterConfig: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
    if (!adapterType || !LOCAL_ADAPTER_TYPES.has(adapterType)) return adapterConfig;
    const instructionsFilePathKey = DEFAULT_INSTRUCTIONS_PATH_KEYS[adapterType] ?? null;
    const cwd = asNonEmptyString(adapterConfig.cwd);
    return seedRoleTemplateFiles({
      role,
      cwd,
      adapterConfig,
      instructionsFilePathKey: instructionsFilePathKey ?? undefined,
    });
  }
  ...
  router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
    ...
    const requestingAgent = await assertCanCreateAgentsForCompany(req, companyId, {
      requestedRole: req.body.role ?? null,
      companyEngineerHeadcount: company.engineerHeadcount ?? 3,
    });
    ...
    const mkdirReadyAdapterConfig = await ensureAdapterWorkingDirectory(
      hireInput.adapterType,
      normalizedAdapterConfig,
      {
        companyId,
    ...
    const initializedAdapterConfig = await initializeAgentRoleWorkspace(
      hireInput.adapterType,
      hireInput.role,
      mkdirReadyAdapterConfig,
    );
    ...
    const normalizedReportsTo = await resolveDefaultReportsTo(
      companyId,
      hireInput.role ?? null,
    ...
    const normalizedHireInput = {
      ...
      adapterConfig: initializedAdapterConfig,
    };
    const architectAutoApproveEngineerHire =
      req.actor.type === "agent" &&
      requestingAgent?.role === "architect" &&
      normalizedHireInput.role === "engineer" &&
      (await countActiveEngineers(companyId)) < (company.engineerHeadcount ?? 3);
    const requiresApproval = company.requireBoardApprovalForNewAgents && !architectAutoApproveEngineerHire;
    ...
    await logActivity(db, {
      ...
      details: {
        ...
        role: agent.role,
        requiresApproval,
        architectAutoApproved: architectAutoApproveEngineerHire,
        engineerHeadcount: company.engineerHeadcount ?? 3,
  ...
  router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
    ...
    await ensureInitialAgentSetupIssue(db, {
      companyId: agent.companyId,
      agentId: agent.id,
      role: agent.role,
    });
  ...
  router.patch("/agents/:id/permissions", validate(updateAgentPermissionsSchema), async (req, res) => {
    ...
    if (req.actor.type === "agent") {
      ...
      if (actorAgent.role !== "manager") {
        res.status(403).json({ error: "Only Manager can manage permissions" });
        return;
      }
    }
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    ...
    if (touchesAdapterConfiguration) {
      ...
      patchData.adapterConfig = await ensureAdapterWorkingDirectory(
        requestedAdapterType,
  ...
  router.get("/issues/:issueId/live-runs", async (req, res) => {
    ...
    const isIdentifier = /^[A-Z0-9_]+-\d+$/i.test(rawId);
