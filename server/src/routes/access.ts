...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "teamclaw" && normalized !== "teamclaw-create-agent")
    return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.teamclawApiUrl)) {
    const legacyTeamClawApiUrl = nonEmptyTrimmedString(input.teamclawApiUrl);
    if (legacyTeamClawApiUrl) merged.teamclawApiUrl = legacyTeamClawApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    teamclawApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.teamclawApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawTeamClawApiUrl =
    typeof defaults.teamclawApiUrl === "string"
      ? defaults.teamclawApiUrl.trim()
      : "";
  if (rawTeamClawApiUrl) {
    try {
      const parsedTeamClawApiUrl = new URL(rawTeamClawApiUrl);
      if (
        parsedTeamClawApiUrl.protocol !== "http:" &&
        parsedTeamClawApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_teamclaw_api_url_protocol",
          level: "warn",
          message: `teamclawApiUrl must use http:// or https:// (got ${parsedTeamClawApiUrl.protocol}).`
        });
      } else {
        normalized.teamclawApiUrl = parsedTeamClawApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_teamclaw_api_url_configured",
          level: "info",
          message: `teamclawApiUrl set to ${parsedTeamClawApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_teamclaw_api_url_invalid",
        level: "warn",
        message: `Invalid teamclawApiUrl: ${rawTeamClawApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your TeamClaw host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "TeamClaw is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Add "${apiHost}" to server.allowedHostnames in config.json, then restart TeamClaw.`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/teamclaw";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/teamclaw-claimed-api-key.json and load TEAMCLAW_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: teamclawApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the TeamClaw hostname is reachable and listed in server.allowedHostnames in config.json."
            : "Ensure OpenClaw can reach this TeamClaw API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/teamclaw/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # TeamClaw OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          teamclawApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, TeamClaw generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "teamclawApiUrl": "https://teamclaw-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in TeamClaw before key claim is allowed.
    ...
    - ~/.openclaw/workspace/teamclaw-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/teamclaw-claimed-api-key.json

    And set the TEAMCLAW_API_KEY and TEAMCLAW_API_URL in your environment variables as specified here:
    ...
    Then set TEAMCLAW_API_KEY and TEAMCLAW_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install TeamClaw skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your TEAMCLAW_API_URL to the top of your skill and note the path to your TEAMCLAW_API_URL
    ...
    ${
      ...
      "Ensure TeamClaw is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested TeamClaw base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.teamclawApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - add the hostname to server.allowedHostnames in config.json
      - then restart TeamClaw and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@teamclaw.local";
...
export function resolveJoinRequestAgentManagerId(
  ...
  const rootManager = managerCandidates.find(
    (candidate) => candidate.reportsTo === null
  );
  return (rootManager ?? managerCandidates[0] ?? null)?.id ?? null;
}
...
export function accessRoutes(
  ...
  router.post("/board-claim/:token/claim", async (req, res) => {
    ...
    if (
      ...
      (req.actor.source !== "session" && req.actor.source !== "pam_session") ||
      !req.actor.userId
    ) {
  ...
  async function assertCanGenerateOpenClawInvitePrompt(
    ...
    if (req.actor.type === "agent") {
      ...
      if (actorAgent.role !== "manager") {
        throw forbidden("Only Manager agents can generate OpenClaw invite prompts");
      }
      return;
    }
    if (req.actor.type !== "board") throw unauthorized();
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "teamclaw", path: "/api/skills/teamclaw" },
        {
          name: "teamclaw-create-agent",
          path: "/api/skills/teamclaw-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              teamclawApiUrl: req.body.teamclawApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.teamclawApiUrl &&
          !persistedDefaults.teamclawApiUrl
        ) {
          missingPersistedFields.push("teamclawApiUrl");
        }
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (existing.requestType === "human") {
        ...
        if (!managerId) {
          throw conflict(
            "Join request cannot be approved because this company has no active Manager"
          );
        }
