...
import { extractProjectMentionIds } from "@teamclawai/shared";
...
export function issueService(db: Db) {
  function deriveIssueIdentifierPrefix(projectName: string | null | undefined) {
    const normalized = (projectName ?? "default")
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_+|_+$/g, "");
    return (normalized || "DEFAULT").slice(0, 16);
  }

  async function findDefaultManagerAssignee(companyId: string, dbOrTx: any = db) {
    return dbOrTx
      .select({
        id: agents.id,
      })
  ...
  async function findDefaultProjectId(companyId: string, dbOrTx: any = db) {
    return dbOrTx
      .select({
        id: projects.id,
      })
  ...
  return {
    list: async (companyId: string, filters?: IssueFilters) => {
      ...
      const rows = await db
        ...
        .orderBy(hasSearch ? asc(searchOrder) : desc(issues.issueNumber), desc(issues.issueNumber), desc(issues.updatedAt));
    ...
    getByIdentifier: async (identifier: string) => {
      const row = await db
        .select()
        .from(issues)
        .where(eq(issues.identifier, identifier.trim().toLowerCase()))
        .then((rows) => rows[0] ?? null);
      if (!row) return null;
      const [enriched] = await withIssueLabels(db, [row]);
      return enriched;
    ...
    create: async (
      ...
      return db.transaction(async (tx) => {
        if (!issueData.projectId) {
          const defaultProjectId = await findDefaultProjectId(companyId, tx);
          if (defaultProjectId) {
            issueData.projectId = defaultProjectId;
          }
        }
        if (!issueData.assigneeAgentId && !issueData.assigneeUserId && issueData.status !== "done" && issueData.status !== "cancelled") {
          const managerId = await findDefaultManagerAssignee(companyId, tx);
          if (managerId) {
            issueData.assigneeAgentId = managerId;
          }
        }
        const hasAssignee = Boolean(issueData.assigneeAgentId || issueData.assigneeUserId);
        if (hasAssignee && (!issueData.status || issueData.status === "backlog")) {
        ...
        const projectName = issueData.projectId
          ? await tx
        ...
        const identifier = `${deriveIssueIdentifierPrefix(projectName)}-${issueNumber}`;
    ...
    update: async (id: string, data: Partial<typeof issues.$inferInsert> & { labelIds?: string[] }) => {
      ...
      return db.transaction(async (tx) => {
        let effectiveAssigneeAgentId = nextAssigneeAgentId;
        let effectiveAssigneeUserId = nextAssigneeUserId;
        const effectiveStatus = (patch.status ?? existing.status) as string;

        if (issueData.projectId === undefined || issueData.projectId === null) {
          const defaultProjectId = await findDefaultProjectId(existing.companyId, tx);
          if (defaultProjectId) {
        ...
        if (
          ...
          const managerId = await findDefaultManagerAssignee(existing.companyId, tx);
          if (managerId) {
            effectiveAssigneeAgentId = managerId;
            patch.assigneeAgentId = managerId;
            patch.assigneeUserId = null;
          }
        }

        const nextHasAssignee = Boolean(effectiveAssigneeAgentId || effectiveAssigneeUserId);
        if (nextHasAssignee && (patch.status === undefined ? existing.status === "backlog" : patch.status === "backlog")) {
          patch.status = "todo";
        }
