...
import type { AdapterRuntimeServiceReport } from "@teamclawai/adapter-utils";
import type { Db } from "@teamclawai/db";
import { workspaceRuntimeServices } from "@teamclawai/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.TEAMCLAW_WORKSPACE_CWD = input.worktreePath;
  env.TEAMCLAW_WORKSPACE_PATH = input.worktreePath;
  env.TEAMCLAW_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.TEAMCLAW_WORKSPACE_BRANCH = input.branchName;
  env.TEAMCLAW_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.TEAMCLAW_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.TEAMCLAW_WORKSPACE_SOURCE = input.base.source;
  env.TEAMCLAW_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.TEAMCLAW_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.TEAMCLAW_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.TEAMCLAW_PROJECT_ID = input.base.projectId ?? "";
  env.TEAMCLAW_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.TEAMCLAW_AGENT_ID = input.agent.id;
  env.TEAMCLAW_AGENT_NAME = input.agent.name;
  env.TEAMCLAW_COMPANY_ID = input.agent.companyId;
  env.TEAMCLAW_ISSUE_ID = input.issue?.id ?? "";
  env.TEAMCLAW_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.TEAMCLAW_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".teamclaw", "worktrees");
