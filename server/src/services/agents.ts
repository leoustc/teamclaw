...
import { isUuidLike, normalizeAgentUrlKey } from "@teamclawai/shared";
...
export function agentService(db: Db) {
  ...
  async function initializeApprovedAgentWorkspace(agent: typeof agents.$inferSelect) {
    if (!agent.adapterType || !LOCAL_ADAPTER_TYPES.has(agent.adapterType)) {
      return normalizeAgentRow(agent);
    }
    const adapterConfig = isPlainRecord(agent.adapterConfig) ? agent.adapterConfig : {};
    const nextAdapterConfig = await seedRoleTemplateFiles({
      role: agent.role,
      cwd: typeof adapterConfig.cwd === "string" ? adapterConfig.cwd : null,
      adapterConfig,
      instructionsFilePathKey: DEFAULT_INSTRUCTIONS_PATH_KEYS[agent.adapterType],
    });
    if (jsonEqual(nextAdapterConfig, adapterConfig)) {
      return normalizeAgentRow(agent);
    }
    return db
      .update(agents)
      ...
      .returning()
      .then((rows) => normalizeAgentRow(rows[0] ?? agent));
  }
  ...
  return {
    ...
    activatePendingApproval: async (id: string) => {
      ...
      return updated ? initializeApprovedAgentWorkspace(updated) : null;
    },
