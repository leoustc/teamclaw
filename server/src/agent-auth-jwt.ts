...
function jwtConfig() {
  const secret = process.env.TEAMCLAW_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.TEAMCLAW_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.TEAMCLAW_AGENT_JWT_ISSUER ?? "teamclaw",
    audience: process.env.TEAMCLAW_AGENT_JWT_AUDIENCE ?? "teamclaw-api",
  };
}
