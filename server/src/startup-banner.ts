...
import { resolveTeamClawConfigPath, resolveTeamClawEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.TEAMCLAW_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.TEAMCLAW_AGENT_JWT_SECRET === "string" ? parsed.TEAMCLAW_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: `missing (set TEAMCLAW_AGENT_JWT_SECRET in ${envFilePath})`,
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveTeamClawEnvPath();
