...
name: release
description: >
  Coordinate a full TeamClaw release across engineering verification, npm,
...
- downstream announcement / operator follow-up tasks
...
7. If running through TeamClaw, you have issue context for status updates and follow-up task creation.
...
TeamClaw now uses this release model:
...
4. Smoke test the canary in the server-style local workflow
...
After publish, verify the published canary version:
...
2. UI loads
3. company/dashboard flow works
4. basic issue and agent flows work
...
- any release summary in TeamClaw issue context
