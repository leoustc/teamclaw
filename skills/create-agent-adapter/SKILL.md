...
description: >
  Technical guide for creating a new TeamClaw agent adapter. Use when building
  a new adapter package, adding support for a new AI coding tool (e.g. a new
  local coding agent, API-based agent, or custom process), or when modifying the adapter
...
# Creating a TeamClaw Agent Adapter

An adapter bridges TeamClaw's orchestration layer to a specific AI agent runtime (Claude Code, Codex, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **two consumers**: the server and the UI.
...
Two registries consume adapter modules:
...
## 2. Shared Types (`@teamclawai/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@teamclawai/adapter-utils` (types) or `@teamclawai/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell TeamClaw to forget the stored session
}
...
**package.json** — must use the three-export convention:

```json
{
  "name": "@teamclawai/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@teamclawai/adapter-utils": "workspace:*"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
  }
...
This file is imported by both consumers (server and UI). Keep it dependency-free (no Node APIs, no React).
...
The `agentConfigurationDoc` is read by LLM agents (including TeamClaw agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@teamclawai/adapter-utils/server-utils`)
2. **Build environment** — call `buildTeamClawEnv(agent)` then layer in `TEAMCLAW_RUN_ID`, context vars (`TEAMCLAW_TASK_ID`, `TEAMCLAW_WAKE_REASON`, `TEAMCLAW_WAKE_COMMENT_ID`, `TEAMCLAW_APPROVAL_ID`, `TEAMCLAW_APPROVAL_STATUS`, `TEAMCLAW_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `TEAMCLAW_AGENT_ID` | `agent.id` |
| `TEAMCLAW_COMPANY_ID` | `agent.companyId` |
| `TEAMCLAW_API_URL` | Server's own URL |
| `TEAMCLAW_RUN_ID` | Current run id |
| `TEAMCLAW_TASK_ID` | `context.taskId` or `context.issueId` |
| `TEAMCLAW_WAKE_REASON` | `context.wakeReason` |
| `TEAMCLAW_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `TEAMCLAW_APPROVAL_ID` | `context.approvalId` |
| `TEAMCLAW_APPROVAL_STATUS` | `context.approvalStatus` |
| `TEAMCLAW_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `TEAMCLAW_API_KEY` | `authToken` (if no explicit key in config) |
...
After creating the adapter package, register it in both consumers:

### 4.1 Server Registry (`server/src/adapters/registry.ts`)

```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@teamclawai/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@teamclawai/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use TeamClaw API
...
import { parseMyAgentStdoutLine } from "@teamclawai/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@teamclawai/adapter-my-agent/ui";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so TeamClaw wipes the stale session
...
Import from `@teamclawai/adapter-utils/server-utils`:
...
| `buildTeamClawEnv(agent)` | Standard `TEAMCLAW_*` env vars |
...
- Package name: `@teamclawai/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your TeamClaw work."`
...
### TeamClaw Skills Injection

TeamClaw ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `teamclaw` API skill and the `teamclaw-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with TeamClaw internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("teamclaw-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "teamclaw-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(TEAMCLAW_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(TEAMCLAW_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks TeamClaw skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The TeamClaw skills (like `teamclaw` and `teamclaw-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the TeamClaw API to report status), use explicit instructions in the prompt template: "Use the teamclaw skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between TeamClaw's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `TEAMCLAW_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the TeamClaw API and GitHub should not have access to arbitrary hosts.
...
- [ ] `packages/adapters/<name>/package.json` with three exports (`.`, `./server`, `./ui`)
