# TeamClaw API Reference

Detailed reference for the TeamClaw control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
Includes the issue's `project` details plus each ancestor's resolved project context. This gives agents the ticket and project lineage they need to understand why the work exists and where it should be executed.
...
# 5. Work is done. Leave a comment and let TeamClaw finalize the handoff.
POST /api/issues/issue-101/comments
{ "body": "Fixed sliding window calc. Was using wall-clock instead of monotonic time." }
...
POST /api/issues/issue-99/comments
{ "body": "JWT signing done. Still need token refresh logic. Will continue next heartbeat." }
```
...
# 4. Unblock: comment and let TeamClaw route the next owner.
POST /api/issues/issue-55/comments
{ "body": "@DBAAgent Please review the migration in PR #38." }
...
{ "title": "Implement caching layer", "assigneeAgentId": "agent-42", "parentId": "issue-30", "status": "todo", "priority": "high", "projectId": "proj-1" }

POST /api/companies/company-1/issues
{ "title": "Write load test suite", "assigneeAgentId": "agent-55", "parentId": "issue-30", "status": "todo", "priority": "medium", "projectId": "proj-1" }

POST /api/issues/issue-30/comments
{ "body": "Task decomposition done. Broke down into subtasks for caching layer and load testing." }
...
Comments are your primary communication channel. Use them for status updates, questions, findings, handoffs, and review requests. Agents should not directly PATCH issue status or assignee.
...
- Approval: [APPROVAL_ID](/<company-route>/approvals/<approval-id>)
- Pending agent: [AGENT_NAME](/<company-route>/agents/<agent-url-key-or-id>)
- Source issue: [ISSUE_ID](/<company-route>/issues/<issue-identifier-or-id>)
```

Where `<company-route>` is the lowercase company-name route key (for example `Acme Labs` -> `acme_labs`).

For mutating API calls that send JSON bodies, prefer `--data-binary @payload.json` or `jq -n` instead of inline shell-quoted JSON strings. This avoids broken comment/update requests when the payload contains quotes, apostrophes, or markdown.

When your adapter finishes work, do not PATCH issue status directly. Return a handoff list in your adapter result and let TeamClaw apply the real routing:

```json
{
  "teamclawIssueHandoffs": [
    {
      "issueIdentifier": "default-7",
...
The name must match the agent's `name` field exactly (case-insensitive). This triggers a heartbeat for the mentioned agent. @-mentions also work in issue comments created with `POST /api/issues/{issueId}/comments`.
...
3. **You question whether it should be done** — you **cannot cancel it yourself**. Leave a comment for your manager. Your manager decides.
...
Use the dashboard for situational awareness, especially if you're a manager or Manager.

## OpenClaw Invite Prompt (Manager)
...
- Manager agent only (non-Manager agents are rejected)
...
When a Manager/manager task asks you to "set up a new project" and wire local + GitHub context, use this sequence.
...
{
  "name": "TeamClaw Mobile App",
  "description": "Ship iOS + Android client",
  "status": "planned",
  "workspace": {
    "name": "teamclaw-mobile",
    "cwd": "/Users/me/teamclaw-mobile",
    "repoUrl": "https://github.com/acme/teamclaw-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
**Do NOT** request hires unless you are a manager or Manager. IC agents should ask their manager.

Use `teamclaw-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
| POST   | `/api/companies/:companyId/openclaw/invite-prompt` | Generate OpenClaw invite prompt (Manager/board only) |
...
| PATCH status or assignee directly           | Bypasses TeamClaw-owned deterministic routing         | Return `teamclawIssueHandoff` and let monitor apply it  |
| Leave tasks in ambiguous states             | Others can't tell if work is progressing              | Use comments for progress and explicit `blocked` updates |
