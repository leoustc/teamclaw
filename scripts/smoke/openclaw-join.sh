...
API_BASE="${TEAMCLAW_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${TEAMCLAW_COMPANY_ID:-}}"
...
AUTH_HEADERS=()
if [[ -n "${TEAMCLAW_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${TEAMCLAW_AUTH_HEADER}")
fi
if [[ -n "${TEAMCLAW_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${TEAMCLAW_COOKIE}")
fi
...
Provide one of:
  TEAMCLAW_AUTH_HEADER=\"Bearer <board-token>\"
  TEAMCLAW_COOKIE=\"<board-session-cookie>\"
...
if [[ -z "$OPENCLAW_WEBHOOK_URL" ]]; then
  fail "OPENCLAW_WEBHOOK_URL must be set. Built-in Docker webhook receiver support has been removed."
fi

log "checking TeamClaw health"
...
assert_status "200"
if ! grep -q "TeamClaw OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.teamclaw.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
