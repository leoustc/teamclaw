...
const env = {
  ...process.env,
  TEAMCLAW_UI_DEV_MIDDLEWARE: "true",
};

if (tailscaleAuth) {
  env.TEAMCLAW_DEPLOYMENT_MODE = "authenticated";
  env.TEAMCLAW_DEPLOYMENT_EXPOSURE = "private";
  env.TEAMCLAW_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[teamclaw] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[teamclaw] dev mode: local_trusted (default)");
}
...
async function maybePreflightMigrations() {
  ...
  if (process.env.TEAMCLAW_MIGRATION_PROMPT === "never") return;

  const status = await runPnpm(
    ["--filter", "@teamclawai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
  ...
  const autoApply = process.env.TEAMCLAW_MIGRATION_AUTO_APPLY === "true";
...
if (mode === "watch") {
  env.TEAMCLAW_MIGRATION_PROMPT = "never";
}

const serverScript = mode === "watch" ? "dev:watch" : "dev";
const child = spawn(
  pnpmBin,
  ["--filter", "@teamclawai/server", serverScript, ...forwardedArgs],
