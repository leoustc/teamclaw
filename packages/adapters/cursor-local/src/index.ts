...
Use when:
- You want TeamClaw to run Cursor Agent CLI locally as the agent runtime
...
- TeamClaw auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$teamclaw" and related skills on local runs.
- TeamClaw auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
