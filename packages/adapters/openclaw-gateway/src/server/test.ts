...
import { asString, parseObject } from "@teamclawai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the TeamClaw server host.",
        });
      }
    } catch (err) {
      checks.push({
