...
import { asNumber, asString, buildTeamClawEnv, parseObject } from "@teamclawai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "teamclaw";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "teamclaw";
  if (input.strategy === "run") return `teamclaw:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `teamclaw:issue:${input.issueId}`;
  return fallback;
}
...
function resolveTeamClawApiUrlOverride(value: unknown): string | null {
...
function buildTeamClawEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const teamclawApiUrlOverride = resolveTeamClawApiUrlOverride(ctx.config.teamclawApiUrl);
  const teamclawEnv: Record<string, string> = {
    ...buildTeamClawEnv(ctx.agent),
    TEAMCLAW_RUN_ID: ctx.runId,
  };

  if (teamclawApiUrlOverride) {
    teamclawEnv.TEAMCLAW_API_URL = teamclawApiUrlOverride;
  }
  if (wakePayload.taskId) teamclawEnv.TEAMCLAW_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) teamclawEnv.TEAMCLAW_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) teamclawEnv.TEAMCLAW_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) teamclawEnv.TEAMCLAW_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) teamclawEnv.TEAMCLAW_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    teamclawEnv.TEAMCLAW_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return teamclawEnv;
}

function buildWakeText(payload: WakePayload, teamclawEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/teamclaw-claimed-api-key.json";
  const orderedKeys = [
    "TEAMCLAW_RUN_ID",
    "TEAMCLAW_AGENT_ID",
    "TEAMCLAW_COMPANY_ID",
    "TEAMCLAW_API_URL",
    "TEAMCLAW_TASK_ID",
    "TEAMCLAW_WAKE_REASON",
    "TEAMCLAW_WAKE_COMMENT_ID",
    "TEAMCLAW_APPROVAL_ID",
    "TEAMCLAW_APPROVAL_STATUS",
    "TEAMCLAW_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = teamclawEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = teamclawEnv.TEAMCLAW_API_URL ?? "<set TEAMCLAW_API_URL>";

  const lines = [
    "TeamClaw wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `TEAMCLAW_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load TEAMCLAW_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $TEAMCLAW_API_KEY on every API call.",
    "- Use X-TeamClaw-Run-Id: $TEAMCLAW_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Build your ticket batch from GET /api/companies/$TEAMCLAW_COMPANY_ID/issues?assigneeAgentId=$TEAMCLAW_AGENT_ID&status=todo,in_progress,blocked. If TEAMCLAW_TASK_ID or issue_id (${issueIdHint}) is present and assigned to you, put that ticket first in the batch.`,
    "3) Process the batch step by step:",
    "   - For each actionable issue, POST /api/issues/{issueId}/checkout with {\"agentId\":\"$TEAMCLAW_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    "   - GET /api/issues/{issueId}",
    "   - GET /api/issues/{issueId}/comments",
    "   - Execute the issue instructions exactly.",
    "   - If instructions require a comment, POST /api/issues/{issueId}/comments with {\"body\":\"...\"}.",
    "   - Never mark success states directly. Return TeamClaw handoff proposals instead and let TeamClaw's internal monitor route the next owner.",
    "4) At the end of the run, return a JSON array named teamclawIssueHandoffs with one entry per processed issue. Each entry should include issueId or issueIdentifier plus proposedStatus, optional nextAgentRole/nextAgentName/nextAgentId, and note.",
    "",
...
function buildStandardTeamClawPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  teamclawEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateTeamClaw = parseObject(payloadTemplate.teamclaw);
  const workspace = asRecord(ctx.context.teamclawWorkspace);
  const workspaces = Array.isArray(ctx.context.teamclawWorkspaces)
    ? ctx.context.teamclawWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.teamclawRuntimeServiceIntents)
    ? ctx.context.teamclawRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardTeamClaw: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: teamclawEnv.TEAMCLAW_API_URL ?? null,
  };

  if (workspace) {
    standardTeamClaw.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardTeamClaw.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardTeamClaw.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "teamclaw-complete");
    this.ws = null;
  }
...
function extractEmbeddedTeamClawResult(text: string | null): Record<string, unknown> | null {
  if (!text) return null;

  const direct = tryParseJsonObject(text.trim());
  if (direct && (Array.isArray(direct.teamclawIssueHandoffs) || asRecord(direct.teamclawIssueHandoff))) {
    return direct;
  }
  ...
  for (const match of fencedMatches) {
    ...
    if (parsed && (Array.isArray(parsed.teamclawIssueHandoffs) || asRecord(parsed.teamclawIssueHandoff))) {
      return parsed;
    }
  }
  ...
  if (objectStart >= 0 && objectEnd > objectStart) {
    const parsed = tryParseJsonObject(text.slice(objectStart, objectEnd + 1));
    if (parsed && (Array.isArray(parsed.teamclawIssueHandoffs) || asRecord(parsed.teamclawIssueHandoff))) {
      return parsed;
    }
  }
...
function extractEmbeddedTeamClawResultFromUnknown(value: unknown): Record<string, unknown> | null {
  if (value == null) return null;
  if (typeof value === "string") return extractEmbeddedTeamClawResult(value);
  try {
    return extractEmbeddedTeamClawResult(JSON.stringify(value));
  } catch {
    return null;
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const teamclawEnv = buildTeamClawEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, teamclawEnv);
  ...
  const teamclawPayload = buildStandardTeamClawPayload(ctx, wakePayload, teamclawEnv, payloadTemplate);

  const agentParams: Record<string, unknown> = {
    ...payloadTemplate,
    teamclaw: teamclawPayload,
    message,
    sessionKey,
  ...
  while (true) {
    ...
    try {
      ...
      const embeddedTeamClawResult =
        extractEmbeddedTeamClawResult(summaryFromEvents) ??
        extractEmbeddedTeamClawResult(summaryFromPayload) ??
        extractEmbeddedTeamClawResult(summary) ??
        extractEmbeddedTeamClawResultFromUnknown(latestResultPayload) ??
        extractEmbeddedTeamClawResultFromUnknown(acceptedPayload);
