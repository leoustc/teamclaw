...
Use when:
- You want TeamClaw to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the TeamClaw server.
...
- workspaceRuntime (object, optional): desired runtime service intents; TeamClaw forwards these in a standardized teamclaw.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- teamclawApiUrl (string, optional): absolute TeamClaw base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default teamclaw)

Standard outbound payload additions:
- teamclaw (object): standardized TeamClaw context added to every gateway agent request
- teamclaw.workspace (object, optional): resolved execution workspace for this run
- teamclaw.workspaces (array, optional): additional workspace hints TeamClaw exposed to the run
- teamclaw.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
