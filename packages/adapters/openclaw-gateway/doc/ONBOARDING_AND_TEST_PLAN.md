...
## Scope
This plan is now **gateway-only**. TeamClaw supports OpenClaw through `openclaw_gateway` only.
...
- Manager agent is allowed to invoke the invite prompt endpoint for their own company
5. E2E pass criteria must include the 3 functional task cases.

## Current Product Flow
1. Board/Manager opens company settings.
...
6. OpenClaw claims API key and installs/uses TeamClaw skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a TeamClaw task.
...
```bash
pnpm --filter @teamclawai/server test:run -- openclaw-gateway
pnpm --filter @teamclawai/server typecheck
pnpm --filter @teamclawai/ui typecheck
pnpm --filter teamclawai typecheck
```
