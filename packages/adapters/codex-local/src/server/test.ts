...
export async function testEnvironment(
  ...
  if (isNonEmpty(configOpenAiKey) || isNonEmpty(hostOpenAiKey)) {
    ...
    checks.push({
      ...
      level: "info",
      message: "OPENAI_API_KEY is not set. TeamClaw will rely on local Codex login/session auth if available.",
      hint: "Set OPENAI_API_KEY only if you want API-key auth instead of local Codex login/session auth.",
    });
  }
