...
Use when:
- You want TeamClaw to run Pi (the AI coding agent) locally as the agent runtime
...
- TeamClaw requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/teamclaws/ and resumed with --session.
