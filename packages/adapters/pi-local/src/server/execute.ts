...
const TEAMCLAW_SESSIONS_DIR = path.join(os.homedir(), ".pi", "teamclaws");
...
async function ensurePiSkillsInjected(onLog: AdapterExecutionContext["onLog"]) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[teamclaw] Injected Pi skill "${entry.name}" into ${piSkillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[teamclaw] Failed to inject Pi skill "${entry.name}" into ${piSkillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(TEAMCLAW_SESSIONS_DIR, { recursive: true });
  return TEAMCLAW_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(TEAMCLAW_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (wakeTaskId) env.TEAMCLAW_TASK_ID = wakeTaskId;
  if (wakeReason) env.TEAMCLAW_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.TEAMCLAW_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.TEAMCLAW_APPROVAL_ID = approvalId;
  if (approvalStatus) env.TEAMCLAW_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.TEAMCLAW_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (workspaceCwd) env.TEAMCLAW_WORKSPACE_CWD = workspaceCwd;
  if (workspaceSource) env.TEAMCLAW_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.TEAMCLAW_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.TEAMCLAW_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.TEAMCLAW_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (workspaceHints.length > 0) env.TEAMCLAW_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[teamclaw] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[teamclaw] Loaded agent instructions file: ${resolvedInstructionsFilePath}\n`,
      );
    } catch (err) {
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[teamclaw] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
