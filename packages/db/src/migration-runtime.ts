...
async function ensureEmbeddedPostgresConnection(
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://teamclaw:teamclaw@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "teamclaw");
    return {
      connectionString: `postgres://teamclaw:teamclaw@127.0.0.1:${port}/teamclaw`,
  ...
  const adminConnectionString = `postgres://teamclaw:teamclaw@127.0.0.1:${preferredPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "teamclaw");

  return {
    connectionString: `postgres://teamclaw:teamclaw@127.0.0.1:${preferredPort}/teamclaw`,
