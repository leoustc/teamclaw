...
function resolveTeamClawInstanceId(): string {
  const raw = process.env.TEAMCLAW_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveTeamClawHomeDir(), "instances", resolveTeamClawInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://teamclaw:teamclaw@127.0.0.1:${port}/teamclaw`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveTeamClawHomeDir(), "instances", resolveTeamClawInstanceId(), "data", "backups");
}
