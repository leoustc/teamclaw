...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "teamclaw-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultConfigPath(): string {
  return path.resolve(
    resolveTeamClawHomeDir(),
    "instances",
    resolveTeamClawInstanceId(),
    CONFIG_BASENAME,
  );
}

function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveTeamClawHomeDir(), "instances", resolveTeamClawInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".teamclaw", CONFIG_BASENAME);
...
function resolveTeamClawConfigPath(): string {
  if (process.env.TEAMCLAW_CONFIG?.trim()) {
    return path.resolve(process.env.TEAMCLAW_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveTeamClawEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveTeamClawEnvPath(configPath);
