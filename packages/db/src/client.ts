...
const RETRYABLE_POSTGRES_CONNECTION_ERROR_CODES = new Set([
  "ECONNREFUSED",
  "ECONNRESET",
  "EPIPE",
  "57P03",
...
function isRetryablePostgresConnectionError(error: unknown): boolean {
  if (!error || typeof error !== "object") return false;

  const code = "code" in error && typeof error.code === "string" ? error.code : null;
  if (code && RETRYABLE_POSTGRES_CONNECTION_ERROR_CODES.has(code)) {
    return true;
  }

  const message = "message" in error && typeof error.message === "string" ? error.message.toLowerCase() : "";
  return message.includes("the database system is starting up");
}

export async function waitForPostgresConnection(
  url: string,
  opts?: {
    attempts?: number;
    delayMs?: number;
  ...
  const probe = opts?.probe ?? (async (probeUrl: string) => {
    const sql = createUtilitySql(probeUrl);
    try {
      await sql`select 1`;
    } finally {
      await sql.end();
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      lastError = error;
      if (!isRetryablePostgresConnectionError(error) || attempt === attempts) {
        throw error;
      }
      await sleep(delayMs);
    }
  ...
  throw lastError instanceof Error ? lastError : new Error("Timed out waiting for PostgreSQL connection");
}
...
async function applyPendingMigrationsManually(
  ...
  await waitForPostgresConnection(url);
